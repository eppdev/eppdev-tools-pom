 ...
 */
package cn.eppdev.tools.web.webauth.springmvc;
...
import cn.eppdev.tools.web.webauth.anno.AuthEntity;
import cn.eppdev.tools.web.webauth.anno.AuthLogin;
import cn.eppdev.tools.web.webauth.anno.AuthModule;
import cn.eppdev.tools.web.webauth.service.AuthorityService;
import cn.eppdev.tools.web.webauth.entity.UserProps;
import cn.eppdev.tools.web.webauth.service.AllPassAuthorityServiceImpl;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.web.method.HandlerMethod;
import org.springframework.web.servlet.handler.HandlerInterceptorAdapter;

import cn.eppdev.tools.web.webauth.entity.DefaultUserProps;
...
public class DefaultAuthorityInteceptor extends HandlerInterceptorAdapter {
	static Logger logger = LoggerFactory.getLogger(DefaultAuthorityInteceptor.class);

	/**
	 * 权限验证的服务，若useService为true，或者设置为要验证每个操作对象的权限，且useServiceForId为true时才使用
	 * <br />
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		AuthModule authModule = handlerMethod.getMethodAnnotation(AuthModule.class);
		AuthEntity authEntity = handlerMethod.getMethodAnnotation(AuthEntity.class);
		AuthLogin authLogin = handlerMethod.getMethodAnnotation(AuthLogin.class);
		logger.debug("authEntity: {}, authModule: {}", authEntity, authModule);

		if (auth(authEntity, request) && auth(authModule, request) && auth(authLogin, request)) {
			return true;
		}
	 ...
	 * @return
	 */
	private boolean auth(AuthLogin authLogin, HttpServletRequest request) {
		// 没有@AuthLogin注解或者已登录，则返回成功
		if (null == authLogin || null != request.getSession().getAttribute(userPropsKey)) {
			return true;
		}
		return false;
	}
	...
	private boolean auth(AuthModule authModule, HttpServletRequest request) {
		...
		if (useServiceForModule) {
			...
			return authorityService.validate(userId, authModule.module(), authModule.operType());
		} else {
			Object obj = request.getSession().getAttribute(userPropsKey);
			if (obj == null || !(obj instanceof DefaultUserProps)) {
				return false;
			}
			return ((DefaultUserProps) obj).hasAuth(authModule.module(), authModule.operType());
		}
	}
	...
	private boolean auth(AuthEntity authEntity, HttpServletRequest request) {
		...
		if (useServiceForId) {
			return authorityService.validate(userId, authEntity.module(), authEntity.operType(), entityId);
		} else {
			Object obj = request.getSession().getAttribute(userPropsKey);
			if (obj == null || !(obj instanceof DefaultUserProps)) {
				return false;
			}
			return ((DefaultUserProps) obj).hasAuth(authEntity.module(), authEntity.operType(), entityId);
		}
	}
	...
	private int getEntityId(String requestURI, String prefix) {
		...
		if (requestURI.contains(prefix)) {
			String afterStr = requestURI.substring(requestURI.indexOf(prefix) + prefix.length());
			if (afterStr != null && afterStr.length() > 0) {
				String idStr;
				if (afterStr.contains("/")) {
					idStr = afterStr.substring(0, afterStr.indexOf("/"));
				} else if (afterStr.contains(".")) {
					idStr = afterStr.substring(0, afterStr.indexOf("."));
				} else {
					idStr = afterStr;
				}
